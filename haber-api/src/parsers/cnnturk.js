...
async function parseCNNTurkNews() {
    ...
    const newsSite = await NewsSite.findOne({
        "_id": constants.cnnTURKSiteID
    })

    if (!newsSite) {
        console.log("News Site (CNNTürk) doesn't exists in database.")
        return
    }
    ...
    if (rss.length == 0) {
        console.log("News Site (CNNTürk) doesn't have RSS feeds.")
        return
    }

    for await (const rssSource of rss) {
    ...
    try {
        ...
        for await (const item of allNews) {
            ...
            if (!isExists && !isFailed) {
                ...
                try {
                    newBody = JSON.parse(onlyJson.get()[1].children[0].data).articleBody.trim()
                } catch {
